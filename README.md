...
![Version: 0.12.0-rc1](https://img.shields.io/badge/Version-0.12.0--rc1-informational?style=flat-square)
...
| image.tag | string | `"1.29.5@sha256:341bf0f3ce6c5277d6002cf6e1fb0319fa4252add24ab6a0e262e0056d313208"` | Docker image tag. Per-workload image overrides are supported via either a full image string (e.g. `deployment.image: my-registry/my-repo:v1.0.0`) or an object that inherits from the top-level `image` config (e.g. `deployment.image.repository: my-repo`). Tag fallback order: resource tag → top-level tag → appVersion → empty (no tag). If registry is omitted, `docker.io` is used. On air-gapped clusters set `image.registry` explicitly. Digest-pinned images must use the full string form (`deployment.image: "registry/repo@sha256:abc123"`). |
