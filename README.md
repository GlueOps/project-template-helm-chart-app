...
| configMap.configs | map | `nil` | ConfigMaps to create, keyed by name suffix. Each config sets exactly one of `data` (multiline string, rendered with Helm templating via `tpl`) or `dataMap` (key/value map of flat scalars, rendered literally without `tpl`). Prefer `dataMap` when overriding values per environment: its keys merge individually across values files, a `dataMap` key set to `null` in an overriding file is removed, and a whole config set to `null` removes that ConfigMap (`data` is replaced as a whole). Quote any `dataMap` key or value that YAML could type - booleans, anything numeric-looking, version strings - the comments under `configs` in values.yaml list every coercion. To migrate a config from `data` to `dataMap`, convert the base values file AND every environment overlay for that config in the same commit - mixing `data` and `dataMap` for one config is an error in either direction. |
