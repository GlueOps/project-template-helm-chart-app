...
| configMap.configs | string | `nil` | Data for the ConfigMap. Each config takes either `data` (multiline string, rendered with Helm templating via `tpl`) or `dataMap` (key/value map, rendered literally without `tpl`). Prefer `dataMap` when overriding values per environment: its keys merge individually across values files (e.g. base-values.yaml + env values.yaml), while `data` is replaced as a whole. Setting a `dataMap` key to `null` in an overriding values file removes it; setting a whole config to `null` removes that ConfigMap. `dataMap` values must be flat scalars (string/number/bool) - nested maps or lists are rejected. Keys must match [-._a-zA-Z0-9]+ (max 253 chars, not '.', must not start with '..'). IMPORTANT - quote any dataMap key or value that YAML could type, because Helm's values parser (YAML 1.1) coerces them before the chart ever sees them: booleans as keys OR values (`on: x` becomes the key "true"; `no:` and `off:` both become "false" and silently collide), octal-like (`0755` -> "493"), hex-like (`0x1F` -> "31"), version strings (`1.10` -> "1.1"), and integers above 2^53, which lose precision in the parser itself. Unquoted numbers below 0.000001 or at/above 1e21 render in scientific notation (`0.0000001` -> "1e-7"). A bare `KEY:` is null and is dropped - use `KEY: ""` for an intentionally empty value. If some values need Helm templating, keep those in a separate config using `data` - each config renders its own ConfigMap. Note: `secret.secrets.<name>.data` and `externalSecret` data are already key/value maps, so they merge per-key out of the box. |
